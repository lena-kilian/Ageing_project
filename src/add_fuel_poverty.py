...
for fp in fp_defs:
    ...
    for item in list(ghg_data.keys()):
        ...
        mean_w[vars_mean_w] = mean_w[vars_mean_w].apply(lambda x: x*mean_w['weight'])
        ...
        mean = mean[vars_mean].apply(lambda x: x/(mean['weight'] * mean['no_people']))
        
        count = cp.copy(temp)
        vars_count = ['GOR', 'OA class 1', 'home_ownership', 'ethnicity hrp', 'ethnicity partner hrp']
        count = count[vars_count + ['weight']].reset_index()
        count_results = count.groupby(['hhd_type', 'fuel_poor', 'hhd_type_1_gender'])[['weight']].sum()
        for item in vars_count:
            temp2 = cp.copy(count)
            temp2['count_' + item] = 1
            temp2 = temp2.groupby(['hhd_type', 'fuel_poor', 'hhd_type_1_gender', item])[['count_' + item, 'weight']].sum()
            temp2['count_' + item] = temp2['count_' + item] * temp2['weight']
            temp2 = temp2[['count_' + item]].unstack(item).fillna(0)
        ...
        demographics[fp] = demographics[fp].append(temp)
